...
function EventInfoView({ event, goBack, openTeam }: {event:any;goBack:()=>void;openTeam:(team:any)=>void}) {
  ...
  return <section className="mx-auto max-w-[1440px] px-5 py-10 lg:px-8">
    ...
    <nav className="mt-4 flex overflow-x-auto border border-white/10 bg-[#101319]" aria-label="Event information">{tabs.map(([key,label])=><button key={key} onClick={()=>setTab(key)} className={`whitespace-nowrap border-r border-white/10 px-5 py-4 text-xs font-semibold uppercase tracking-wider transition ${tab===key?'bg-[#ed2b3a] text-white':'text-white/45 hover:bg-white/[.04] hover:text-white'}`}>{label}{key==='awards'&&awards.length>0?` ${awards.length}`:''}</button>)}</nav>
    <div key={tab} className="view-enter mt-4">
      ...
      {loading&&<article className="border border-white/10 bg-[#101319] p-8 text-sm text-white/55">Loading official event information…</article>}
...
function EventResults({divisions,skills,resultsTab,setResultsTab,skillsMode,setSkillsMode,qualificationView,setQualificationView}:any) {
  ...
  return <article className="border border-white/10 bg-[#101319] p-6"><SectionTitle eyebrow="Official competition data" title="Results" />
...
function StatRankingsView({ teams: rows, openTeam }: { teams: any[]; openTeam:(t:any)=>void }) {
  ...
  return <section className="mx-auto max-w-[1440px] px-5 py-10 lg:px-8"><PageHead eyebrow="Performance leaderboards" title="Who leads every part of the game?" copy="Go beyond the overall world ranking and discover the teams setting the standard in scoring, defense, autonomous play and strategy." />
    ...
    {visibleCount<ranking.length&&<button onClick={()=>setVisibleCount(count=>count+250)} className="mt-4 w-full border border-white/10 bg-[#101319] py-3 text-sm text-white/50 hover:border-white/30 hover:text-white">Show more teams · {ranking.length-visibleCount} remaining</button>}
...
function SeasonRatingChart({data,onOpenEvent}:{data:any[];onOpenEvent:(point:any)=>void}) {
  ...
  if(!data.length)return <article className="mt-6 border border-white/10 bg-[#101319]"><SectionTitle eyebrow="Season progression" title="Tournament rating movement" /><Empty text="No scored tournament matches are available for this season." /></article>;
  ...
  return <article className="mt-6 border border-white/10 bg-[#101319]">
    ...
    <div className="p-5">
      <div className="mb-5 flex flex-col gap-2 rounded-r-lg border-l-2 border-[#ed2b3a] bg-white/[.025] p-4 sm:flex-row sm:items-center">
        <div className="min-w-0">
          ...
          <button onClick={()=>onOpenEvent(detail)} className="mt-1 block max-w-full truncate text-left font-semibold hover:text-[#ed2b3a]">{detail.event}</button>
        </div>
...
function TeamView({ team, goBack, backLabel, openEvent }: { team:any; goBack:()=>void;backLabel:string;openEvent:(event:any)=>void }) {
  ...
  return <section><div className="border-b border-white/10 bg-[#0b0d11]"><div className="mx-auto max-w-[1440px] px-5 pb-12 pt-8 lg:px-8"><button onClick={goBack} className="flex items-center gap-2 text-xs text-white/55 hover:text-white"><ArrowLeft className="h-4 w-4" /> Back to {backLabel}</button>
      ...
      <h1 className="team-lockup rise mt-3" style={{'--i':1} as React.CSSProperties}><span className="team-lockup-digits">{numberDigits}</span><span className="team-lockup-suffix">{numberSuffix}</span></h1>
